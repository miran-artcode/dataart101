...
(function (global) {
  ...
  Algos.kmeansArt = function (src, opts) {
    ...
    const maxDim = opts.maxDim || 240;
    const a = ImageAnalysis.analyze(src, { K, space, sampling: 'uniform', N: opts.N || 1, seed: opts.seed || 12345, maxDim: maxDim });
    const sm = small(src, 300);
    // 군집 응집도(평균 색거리) — 작을수록 단순/균일, 클수록 복잡/다채. 데이터로 의미 있는 지표.
    let inertia = 0, denom = 0;
    const d = sm.data.data, P = a.palette;
    for (let i = 0; i < d.length; i += 4) {
      const r = d[i], g = d[i + 1], b = d[i + 2];
      let bd = Infinity;
      for (let k = 0; k < P.length; k++) { const dr = r - P[k].r, dg = g - P[k].g, db = b - P[k].b, dd = dr * dr + dg * dg + db * db; if (dd < bd) bd = dd; }
      inertia += bd; denom++;
    }
    const rmse = Math.sqrt(inertia / Math.max(1, denom));
    return { palette: a.palette, recreate: toCanvas(quantizeTo(sm.data, a.palette)), meta: { K: a.K, requestedK: a.requestedK, maxDim: maxDim, rmse: +rmse.toFixed(1) } };
  };
  ...
  Algos.edges = function (src, opts) {
    opts = opts || {};
    const invert = !(opts.invert === false); // 기본: 흰 바탕에 검은 선
    const threshold = opts.threshold || 0;   // 0~255: 이보다 약한 윤곽은 지움(잡선 제거)
    const strength = opts.strength != null ? opts.strength : 1; // 윤곽 대비 증폭
    ...
    let edgeCount = 0, edgeSum = 0;
    for (let y = 1; y < h - 1; y++) for (let x = 1; x < w - 1; x++) {
      const i = y * w + x;
      const gx = -gray[i - w - 1] - 2 * gray[i - 1] - gray[i + w - 1] + gray[i - w + 1] + 2 * gray[i + 1] + gray[i + w + 1];
      const gy = -gray[i - w - 1] - 2 * gray[i - w] - gray[i - w + 1] + gray[i + w - 1] + 2 * gray[i + w] + gray[i + w + 1];
      let m = Math.hypot(gx, gy) * strength;
      if (m < threshold) m = 0;
      m = m > 255 ? 255 : m;
      if (m > threshold) { edgeCount++; edgeSum += m; }
      const v = invert ? 255 - m : m;
      const j = i * 4; o[j] = o[j + 1] = o[j + 2] = v;
    }
    return { recreate: toCanvas(out), meta: { density: +(edgeCount / Math.max(1, (w - 2) * (h - 2)) * 100).toFixed(1), avg: edgeCount ? +(edgeSum / edgeCount).toFixed(0) : 0 } };
  };
  ...
  Algos.mosaic = function (src, opts) {
    opts = opts || {};
    const cell = opts.cell || 16, shape = opts.shape || 'square', gap = Math.min(opts.gap || 0, cell - 1);
    const sm = small(src, 360), w = sm.w, h = sm.h, d = sm.data.data;
    const out = blank(w, h), octx = out.getContext('2d');
    octx.fillStyle = '#0b0e16'; octx.fillRect(0, 0, w, h);
    let tiles = 0;
    for (let by = 0; by < h; by += cell) for (let bx = 0; bx < w; bx += cell) {
      ...
      const s = cell - gap, ox = bx + gap / 2, oy = by + gap / 2, cx = bx + cell / 2, cy = by + cell / 2;
      if (shape === 'circle') { octx.beginPath(); octx.arc(cx, cy, s / 2, 0, Math.PI * 2); octx.fill(); }
      else if (shape === 'diamond') { octx.beginPath(); octx.moveTo(cx, oy); octx.lineTo(ox + s, cy); octx.lineTo(cx, oy + s); octx.lineTo(ox, cy); octx.closePath(); octx.fill(); }
      else octx.fillRect(ox, oy, s, s);
      tiles++;
    }
    return { recreate: out, meta: { tiles: tiles, cell: cell } };
  };
